(function(root, factory) {
	if (typeof define === 'function' && define.amd) {
		...
		root.ManagedHub = factory(root.Errors);
	}
	...
	ManagedHub.prototype = {
		 ...
		 * 
		 * @throws {Error.NoContainer}
		 *             if no such container is found
		 */
		removeContainer : function(container) {
			var containerId = container.getClientID();
			if (!this._containers[containerId]) {
				throw new Error(Errors.NoContainer);
			}
			container.remove();
		...
		subscribe : function(topic, onData, scope, subscriberData) {
			var promise = new Promise(function(resolve, reject) {
				...
				if (!this._invokeOnSubscribe(topic, null)) {
					reject(new Error(Errors.NotAllowed));
				}
